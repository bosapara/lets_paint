...
for _, v in pairs(v) do
	...
	minetest.register_tool("lets_paint:"..v[1], {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if (npos.y < 400 or npos.y > 430) or (npos.x < 96 or npos.x > 120) or (npos.z < - 127 or npos.z > -103) then
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname=="brush_form" then
		...
		for _, v in pairs(v) do
			...
			if fields[v[1]] and string.match(item, "lets_paint:") then

				player:set_wielded_item("lets_paint:"..v[1])
...
minetest.register_on_punchnode(function(pos, node, puncher, pointed_thing)
	...
	if (npos.y < 400 or npos.y > 430) or (npos.x < 96 or npos.x > 120) or (npos.z < - 127 or npos.z > -103) and string.match(item, "lets_paint:") then
		...
		for _, v in pairs(v) do
			...
			if puncher:get_wielded_item():get_name() == "lets_paint:"..v[1] and node.name ~= "air" and string.match(node.name, "wool:") and not string.match(node.name, "carpet") then
